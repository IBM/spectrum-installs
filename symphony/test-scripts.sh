...
#    1    #       0       #      0     #    shared    # enabled  #   bin   #       no       #   yes    #  yes
#    2    #       0       #      1     #    shared    # enabled  #   rpm   #       yes      #   no     #  no
#    3    #       1       #      0     #    shared    # disabled #   bin   #       no       #   no     #  yes
#    4    #       1       #      0     #    local     # disabled #   bin   #       no       #   no     #  yes
#    5    #       0       #      1     #    local     # enabled  #   rpm   #       yes      #   yes    #  no
#
# USAGE
...
updateParametersGlobal() {
  ...
  local INSTALL_HADOOP_COMPONENTS=$7
  ...
  sed -i "s#export INSTALL_MAPREDUCE=.*#export INSTALL_MAPREDUCE=$INSTALL_HADOOP_COMPONENTS#" $CONF_PARAMETERS_FILE 2>&1 | tee -a $LOG_FILE
...
backupConfig
log "Starting test 1"
updateParametersGlobal $SERVER1_HOSTNAME $USER_ID shared $SHARED_DIR enabled disabled N enabled 10
testInstall 1 "$SERVER1_HOSTNAME"
log "Starting test 2"
updateParametersGlobal $SERVER1_HOSTNAME $USER_ID shared $SHARED_DIR enabled enabled Y disabled 0
updateComputeHosts "$SERVER2_HOSTNAME"
testInstall 2 "$SERVER1_HOSTNAME $SERVER2_HOSTNAME"
log "Starting test 3"
updateParametersGlobal $SERVER1_HOSTNAME $USER_ID shared $SHARED_DIR disabled disabled N disabled 10
...
log "Starting test 4"
updateParametersGlobal $SERVER1_HOSTNAME $USER_ID local $LOCAL_DIR disabled disabled N disabled 10
updateParametersMasterCandidates "$SERVER1_HOSTNAME,$SERVER2_HOSTNAME" $SHARED_DIR/ego-share
updateManagementHosts "$SERVER2_HOSTNAME"
testInstall 4 "$SERVER1_HOSTNAME $SERVER2_HOSTNAME"
log "Starting test 5"
updateParametersGlobal $SERVER1_HOSTNAME $USER_ID local $LOCAL_DIR enabled enabled Y enabled 0
updateComputeHosts "$SERVER2_HOSTNAME"
