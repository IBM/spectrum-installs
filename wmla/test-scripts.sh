...
#
# TEST CASES
# Test id # NB MANAGEMENT # NB COMPUTE # INSTALL_TYPE # DEPLOYMENT_TYPE #   SSL    # bin/rpm # local channel #  airgap  # lab env # lab env OS user  # pssh
#    1    #       0       #      0     #    shared    #       N/A       # enabled  #   bin   #    disabled   # enabled  #   yes   #      no          #  yes
#    2    #       0       #      1     #    shared    #       N/A       # enabled  #   bin   #    disabled   # disabled #   yes   #      yes         #  yes
#    3    #       1       #      0     #    shared    #       N/A       # disabled #   rpm   #    enabled    # disabled #   no    #      N/A         #  no
#    4    #       0       #      1     #    local     #      shared     # enabled  #   bin   #    disabled   # enabled  #   yes   #      yes         #  yes
#    5    #       0       #      1     #    local     #      local      # enabled  #   rpm   #    enabled    # disabled #   yes   #      no          #  no
#
# USAGE
...
# 4. Update DLI_DATASET_SOURCE_DIR and DLI_MODEL_SOURCE_DIR in lab-environment.inc if necessary
# 5. Update parameters in PARAMETERS section at the top of this script
# 6. Execute this script on server defined as SERVER1_HOSTNAME
# 7. Check results in TESTS_DIR
#############################
...
export CONF_LABENV_FILE=`dirname "$(readlink -f "$0")"`/conf/lab-environment.inc
...
cleanup() {
  ...
  then
    log "Deleting lab environment"
    source `dirname "$(readlink -f "$0")"`/conf/lab-environment.inc
    `dirname "$(readlink -f "$0")"`/delete-lab-environment.sh ${LAB_USER_BASE}1 2>&1 | tee -a $LOG_FILE
    ERRORCODE=${PIPESTATUS[0]}
    \cp `dirname "$(readlink -f "$0")"`/logs/delete-lab-environment* $TEST_DIR/logs 2>&1 | tee -a $LOG_FILE
    if [ $ERRORCODE -ne 0 ]
    then
      log "Failed to execute delete-lab-environment.sh script with argument \"${LAB_USER_BASE}1\" (error code: $ERRORCODE), aborting" ERROR
      exit 1
    fi
  fi
  log "Uninstalling cluster"
  ...
  for FILE_TO_REMOVE in $ANACONDA_AIRGAP_INSTALL_DLINSIGHTS_ARCHIVE $ANACONDA_AIRGAP_INSTALL_IG_ARCHIVE_BASENAME*
  do
...
prepareAirgapArchives() {
  ...
  if [ $ERRORCODE -eq 0 ]
  then
    if  [ -f $ANACONDA_AIRGAP_INSTALL_DLINSIGHTS_ARCHIVE ]
    then
      log "Airgap archive for dlinsights created successfully" SUCCESS
      echo "Airgap dlinsights archive preparation: SUCCESS ($DURATION)" >> $TESTS_SUMMARY
    else
      log "Failed to prepare airgap archive for dlinsights (file $ANACONDA_AIRGAP_INSTALL_DLINSIGHTS_ARCHIVE doesn't exist), aborting" ERROR
      echo "Airgap dlinsights archive preparation: FAILED - file $ANACONDA_AIRGAP_INSTALL_DLINSIGHTS_ARCHIVE doesn't exist ($DURATION)" >> $TESTS_SUMMARY
      exit 1
    fi
    if ls ${ANACONDA_AIRGAP_INSTALL_IG_ARCHIVE_BASENAME}* 1> /dev/null 2>&1;
    then
      log "Airgap archive for Instance Groups created successfully" SUCCESS
      echo "Airgap Instance Groups archive preparation: SUCCESS ($DURATION)" >> $TESTS_SUMMARY
    else
      log "Failed to prepare airgap archive for Instance Groups (no file match pattern ${ANACONDA_AIRGAP_INSTALL_IG_ARCHIVE_BASENAME}*), aborting" ERROR
      echo "Airgap Instance Groups archive preparation: FAILED - no file match pattern ${ANACONDA_AIRGAP_INSTALL_IG_ARCHIVE_BASENAME}* ($DURATION)" >> $TESTS_SUMMARY
      exit 1
    fi
  else
...
updateParametersGlobal() {
  ...
  local CLUSTERINSTALL_CREATE_LAB_ENVIRONMENT=${11}
  local LAB_CREATE_OS_USER=${12}
  local PSSH_NBHOSTS_IN_PARALLEL=${13}
  ...
  sed -i "s#export CLUSTERINSTALL_CREATE_LAB_ENVIRONMENT=.*#export CLUSTERINSTALL_CREATE_LAB_ENVIRONMENT=$CLUSTERINSTALL_CREATE_LAB_ENVIRONMENT#" $CONF_PARAMETERS_FILE 2>&1 | tee -a $LOG_FILE
  sed -i "s#export LAB_CREATE_OS_USER=.*#export LAB_CREATE_OS_USER=$LAB_CREATE_OS_USER#" $CONF_LABENV_FILE 2>&1 | tee -a $LOG_FILE
...
restoreConfig
log "Starting test 1"
updateParametersGlobal $SERVER1_HOSTNAME $USER_ID shared shared $SHARED_DIR $SHARED_DIR enabled disabled disabled enabled enabled disabled 10
prepareAirgapArchives
testInstall 1 "$SERVER1_HOSTNAME"
log "Starting test 2"
updateParametersGlobal $SERVER1_HOSTNAME $USER_ID shared shared $SHARED_DIR $SHARED_DIR enabled disabled disabled disabled enabled enabled 10
updateComputeHosts "$SERVER2_HOSTNAME"
testInstall 2 "$SERVER1_HOSTNAME $SERVER2_HOSTNAME"
log "Starting test 3"
updateParametersGlobal $SERVER1_HOSTNAME $USER_ID shared shared $SHARED_DIR $SHARED_DIR disabled enabled enabled disabled disabled disabled 0
...
log "Starting test 4"
updateParametersGlobal $SERVER1_HOSTNAME $USER_ID local shared $LOCAL_DIR $SHARED_DIR enabled disabled disabled enabled enabled enabled 10
updateComputeHosts "$SERVER2_HOSTNAME"
prepareAirgapArchives
testInstall 4 "$SERVER1_HOSTNAME $SERVER2_HOSTNAME"
log "Starting test 5"
updateParametersGlobal $SERVER1_HOSTNAME $USER_ID local local $LOCAL_DIR $SHARED_DIR enabled enabled enabled disabled enabled disabled 0
updateComputeHosts "$SERVER2_HOSTNAME"
