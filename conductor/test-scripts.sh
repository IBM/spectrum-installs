...
#############################
# DESCRIPTION
# This script will execute several installation of Conductor and test correct installation.
...
# 2. Add Conductor installer and entitlement file, and update path in parameters.inc if necessary
...
cleanup() {
  ...
  do
	...
	for FILE_TO_REMOVE in $ANACONDA_AIRGAP_INSTALL_IG_ARCHIVE
	do
...
prepareAirgapArchives() {
  ...
  ERRORCODE=${PIPESTATUS[0]}
	...
	if [ $ERRORCODE -eq 0 ]
  then
    if  [ -f $ANACONDA_AIRGAP_INSTALL_IG_ARCHIVE ]
    then
      log "Airgap archives created successfully" SUCCESS
			echo "Airgap archives preparation: SUCCESS ($DURATION)" >> $TESTS_SUMMARY
    else
      log "Failed to prepare airgap archives (file $ANACONDA_AIRGAP_INSTALL_IG_ARCHIVE doesn't exist), aborting" ERROR
			echo "Airgap archives preparation: FAILED - file $ANACONDA_AIRGAP_INSTALL_IG_ARCHIVE doesn't exist ($DURATION)" >> $TESTS_SUMMARY
      exit 1
    fi
  else
...
checkInstall() {
  ...
  log "Checking install of test $TEST_ID"
	...
	then
		...
		checkAnaconda $IG_ANACONDA_INSTANCE_NAME "$IG_SPARK243_CONDA_ENV_NAME"
