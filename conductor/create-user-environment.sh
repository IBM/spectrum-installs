...
then
	log "Create $IG_SPARK243_CONDA_ENV_NAME conda environment based on the airgap archive package and wait for successful deployment"
  extractCondaEnvironmentsFromArchive $ANACONDA_AIRGAP_INSTALL_IG_ARCHIVE $IG_ANACONDA_INSTANCE_DEPLOY_HOME/anaconda "$IG_SPARK243_CONDA_ENV_NAME"
	discoverCondaEnvironments $IG_ANACONDA_INSTANCE_DEPLOY_HOME/anaconda "$IG_SPARK243_CONDA_ENV_NAME" $ANACONDA_INSTANCE_UUID
else
